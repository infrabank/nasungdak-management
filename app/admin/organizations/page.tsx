...
import { Building2, Clock, CheckCircle, XCircle, ArrowLeft } from 'lucide-react'
...
export default async function OrganizationsPage() {
  ...
  const trialOrgs = organizations.filter(
    (o) => o.trialEndsAt && new Date(o.trialEndsAt) > new Date()
  ).length
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-black text-brutal-black">조직 관리</h1>
        <p className="mt-2 text-brutal-black/70">
          전체 조직 목록을 관리합니다 ({totalOrgs}개)
        </p>
      </div>
