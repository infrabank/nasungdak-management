...
import { getDayOfWeekAnalysis } from './actions'
...
export default async function DayOfWeekPage({
  ...
  const startDate =
    params.startDate || formatDate(threeMonthsAgo, 'yyyy-MM-dd')
  const endDate = params.endDate || formatDate(today, 'yyyy-MM-dd')
  const storeId = normalizeOptionalParam(params.storeId)

  const result = await getDayOfWeekAnalysis(startDate, endDate, storeId)
