...
export async function createOilChange(formData: FormData) {
  try {
    ...
    const rawData = {
      ...
      previousOilUsage: formData.get('previousOilUsage'),
      usageDays: formData.get('usageDays'),
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        unitPrice: validatedData.unitPrice,
        previousOilUsage: validatedData.previousOilUsage,
        usageDays: validatedData.usageDays || null,
        notes: validatedData.notes,
        updatedAt: new Date(),
      })
      .where(and(eq(oilChangeHistory.id, id), isNull(oilChangeHistory.deletedAt)))
...
export async function getOilChangeStats() {
  try {
    // Get total oil changes in the last 30 days
    ...
    const recentChanges = await db
      .select({
        ...
        totalCost: sql`SUM(total_cost)`.mapWith(Number),
      })
      .from(oilChangeHistory)
      .where(
        and(
