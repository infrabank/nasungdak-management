...
export default function IngredientsPageClient({
  ...
  useEffect(() => {
    if (includeOneTime) {
      getIngredients(true).then(setIngredients)
    } else {
      getIngredients(false).then(setIngredients)
    }
  }, [includeOneTime])
  ...
  return (
    <div>
      ...
      {/* One-time filter toggle */}
      <div className="mt-4">
        <label className="inline-flex cursor-pointer items-center gap-2">
          <button
            type="button"
            role="switch"
            aria-checked={includeOneTime}
            onClick={() => setIncludeOneTime(!includeOneTime)}
            className={`relative inline-flex h-6 w-11 shrink-0 items-center border-2 border-brutal-black transition-colors ${
              includeOneTime ? 'bg-brutal-blue' : 'bg-brutal-white'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 border border-brutal-black bg-brutal-black transition-transform ${
                includeOneTime ? 'translate-x-5' : 'translate-x-0.5'
              }`}
            />
          </button>
                  ...
                  ingredients.map((ingredient) => (
                    <tr key={ingredient.id}>
                      <td className="whitespace-nowrap py-4 pl-6 pr-3 text-sm font-medium text-brutal-black">
                        ...
                        {ingredient.isOneTime && (
                          <span className="ml-2 inline-flex border border-brutal-black bg-brutal-pink/50 px-1.5 py-0.5 text-[10px] font-bold text-brutal-black">
                            일회성
                          </span>
                        )}
                      </td>
